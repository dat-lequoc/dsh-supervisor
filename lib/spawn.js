 ...
 * settings allowlist, each entry annotated with its native input modalities
 ...
 * and no parent-model inheritance: an empty allowlist exposes no spawn tool.
 *
...
export async function apply(ctx, config) {
  ...
  // fallback and no parent-model inheritance: an empty/missing list means no
  // worker may start.
  ...
  const mount = () => {
    ...
    disposeTool = ctx.tools.register(defineTool({
      ...
      description:
        ...
        + 'The required `model` argument must be one exact provider/model route from the user-owned worker '
        + 'allowlist; no default or parent model is inherited. Native input modalities are shown beside every '
        + 'allowed route (pick a [text,image] model only when the worker must inspect images). '
      ...
      parameters: {
        ...
        model: {
          ...
          description: `Required worker route. Only settings-approved routes are accepted:\n${allowedRows}`,
        },
        run_in_background: {
          type: 'boolean',
      ...
      async execute(args, exec) {
        ...
        const selected = selectWorkerModel(args.model, allowlistCache)
    ...
    text: () => {
      ...
      const lines = [
        ...
        allowlistCache.length > 0
          ? `- ${toolName} requires one exact \`model\` from this complete worker allowlist; there is no default inheritance:\n${allowlistCache.map(entry => `    ${workerModelLabel(entry)}`).join('\n')}`
          : `- Worker spawning is disabled: no models are allowed. The user must add a route in Settings -> Plugins -> dsh-supervisor; never use another delegation tool to bypass this policy.`,
