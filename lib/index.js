...
const SettingsSchema = z.object({
  ...
  /** Deployment default check-in cadence (minutes, schedule floor is 5).
   * A workspace's .agi/config.json overrides it. */
  wakeMinutes: z.number().step(1).min(5).default(5),
  /** Deployment default question patience (minutes) before the supervisor
   * records an assumption. A workspace's .agi/config.json overrides it. */
