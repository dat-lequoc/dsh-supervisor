 ...
 *
 * 3. FULL STOP. `POST /supervisor/stop?session=…` aborts the active turn and
 *    deletes every schedule reminder inside an exclusive maintenance window,
 *    so nothing re-wakes the agent (the native stop only aborts the turn).
 *
...
/**
 * `POST /supervisor/stop?session=<sessionId>` — FULL stop for one supervisor:
 * aborts the active turn (clearing queued work) and deletes every active
 * schedule reminder, so nothing re-wakes the agent later. Without this, the
 * native stop square only aborts the current turn — the recurring check-in
 * reminder (`wakeMinutes`, floor 5 min) fires again and the agent resumes.
 *
 ...
 *
 * Running workers are NOT killed: each may still wake the supervisor once with
 * its settle report, but with the reminders gone there is no recurring wake.
 */
async function stopHandler(ctx, req, res) {
  ...
  if (agent === undefined) {
    // No live agent: nothing is running and no runtime is armed, so there is
    // nothing to wake until the session is resumed. Report that honestly.
    return send(200, { live: false, cancelled: false, remindersDeleted: 0, note: 'session is not live; reminders (if any) stay dormant until resume' })
  }
  ...
  const idle = () => Promise.race([
    ...
    new Promise(resolvePromise => setTimeout(() => resolvePromise(false), 15000)),
  ])
  if (!await idle()) return send(504, { error: 'agent did not reach idle within 15s; reminders not touched — retry' })

  // runMaintenance throws synchronously while another activity owns the idle
  // phase (e.g. the schedule runtime dispatching); a short retry ladder covers
  // that window.
  for (let attempt = 0; attempt < 3; attempt++) {
    let deleted
    try {
      deleted = await agent.runMaintenance(async () => {
        ...
        // Synchronous append loop inside the exclusive window: every id comes
        // from the fold of this very log a microtask ago, so each delete
        // targets an active schedule by construction.
        for (const id of ids) {
    ...
    try {
      // Durability barrier so the deletes survive a process restart.
      await ctx.sessions.flush(agent.session)
    } catch {
      return send(200, { live: true, cancelled: true, remindersDeleted: deleted.length, ids: deleted, note: 'flush uncertain — deletes applied in memory' })
    }
    return send(200, { live: true, cancelled: true, remindersDeleted: deleted.length, ids: deleted })
  }
