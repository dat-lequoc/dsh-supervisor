...
import { patchLegacyImageLimits } from './image-limits-compat.js'
...
export function apply(ctx, config) {
  ...
  // A locally checked-out rc.7 harness can compose profile-installed rc.8 web
  // packages. rc.8 validates one additional image-limit projection field;
  // adapt the legacy attachment service at our plugin boundary so history
  // remains readable without changing the harness or persisted sessions.
  ctx.inject(['attachments'], (attachmentCtx) => {
    // Returning the disposer transfers ownership to this injected fiber.
    // Passing it to `ctx.effect()` directly would execute it as setup and
    // immediately undo the patch.
    return patchLegacyImageLimits(attachmentCtx.attachments, log)
  })
