  ...
  factory: (require) => {
    ...
    function FeedView(props) {
      ...
      const fullStop = async () => {
        const warning = 'Fully stop this agent?\n\n'
        ...
        try {
          const res = await fetch(`/supervisor/stop?session=${encodeURIComponent(sessionId)}`, { method: 'POST' })
          const body = await res.json().catch(() => null)
          if (res.ok && body != null) {
            setStopState({
              ok: true,
              message: body.live === false
                ? 'not live — nothing armed'
                : `stopped · ${body.remindersDeleted} reminder${body.remindersDeleted === 1 ? '' : 's'} cleared`,
            })
          } else {
      ...
      h('button', {
        ...
        title: 'Abort the current turn AND delete all schedule reminders — no auto-wake. (The native stop square only aborts the turn; the recurring check-in re-runs the agent within wakeMinutes.)',
        style: {
