...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * one worker-effort policy selector, and the three numeric knobs. Edits
    ...
    function makeSupervisorCard(scope) {
      return function SupervisorCard() {
        ...
        const current = draft ?? {
          ...
          workerEffort: typeof value.workerEffort === 'string' ? value.workerEffort : 'runtime/current',
        ...
        const save = async () => {
          ...
          try {
            ...
            await scope.set('workerEffort', draft.workerEffort)
        ...
        const effortModels = current.workerModels.includes('runtime/current')
          ? catalog
          : catalog.filter(model => current.workerModels.includes(`${model.provider}/${model.id}`))
        const effortById = new Map()
        for (const model of effortModels) {
          for (const effort of model.reasoning?.efforts ?? []) {
            if (!effortById.has(effort.id)) effortById.set(effort.id, effort)
          }
        }
        const effortOptions = [...effortById.values()]
        ...
        h('label', { style: { display: 'grid', gap: 4 } },
          h('div', { style: { display: 'flex', alignItems: 'center', gap: 10, fontSize: 13, color: palette.text } },
            h('span', { style: { width: 220, flex: 'none' } }, 'Worker reasoning effort'),
            h('select', {
              value: current.workerEffort,
              disabled,
              onChange: event => edit({ workerEffort: event.target.value }),
              style: {
                minWidth: 260, padding: '5px 8px', borderRadius: 8, fontSize: 13,
                border: `1px solid ${palette.border}`, background: 'transparent', color: palette.text,
              },
            },
            h('option', { value: 'runtime/current' }, 'Same as current main turn (default)'),
            h('option', { value: 'provider/default' }, 'Provider default'),
            !['runtime/current', 'provider/default'].includes(current.workerEffort)
              && !effortById.has(current.workerEffort)
              ? h('option', { value: current.workerEffort }, `${current.workerEffort} (not advertised by allowed models)`)
              : null,
          ...
          h('p', { style: { margin: 0, fontSize: 12, color: palette.dim, lineHeight: 1.45 } },
            'Same as current main turn copies its explicit effort. Provider default sends no effort. A fixed value must be advertised by the exact worker model or spawning is refused before provider I/O.')),
