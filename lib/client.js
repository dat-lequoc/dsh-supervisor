...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function FeedView(props) {
      ...
      // FULL stop: aborts the turn, releases delegated worker branches, and
      // deletes every schedule reminder, so nothing can re-run the agent.
      const fullStop = async () => {
        ...
        const warning = 'Fully stop the supervisor?\n\n'
          + 'Aborts the current turn, stops all running delegated workers (including descendants), '
          + 'and deletes ALL schedule reminders.'
        ...
        try {
          ...
          if (res.ok && body != null) {
            ...
            setStopState({
              ...
              message: `stopped · ${body.workersStopped ?? 0} worker${body.workersStopped === 1 ? '' : 's'} stopped`
                + ` · ${body.remindersDeleted} reminder${body.remindersDeleted === 1 ? '' : 's'} cleared`
      ...
      stopState?.ok === true || stopStatus?.stopped === true || supervisorSessionId == null
        ...
        : h('button', {
            ...
            title: 'Stop the supervisor, its delegated workers, and all schedule reminders. A later manual prompt resumes the supervisor.',
            style: {
